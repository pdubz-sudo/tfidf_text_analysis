...
The words we would like to investigate are first extracted from Project Gutengurg which is a free e-books project.
```{r}
...
manipulate this data so each word is it's own row. We will now turn it into a 
tidy text dataset
```{r}
...
such as "the", "and", "to", "of", etc.
Therefor, we use the tf_idf statistic to weight the words in our collection of
...
In conclusion, we can see the most important words in a particular childrens
book in a collection of children books. As it is evident, the rudimentary words
...
#######################################################################
Just for fun, let's have fun with visuals and make a wordcloud of the MOST redundant words we found in the all of the childrens books. Do these words look
familiar regardless of what document they come from?
```{r}
weighted_words %>%
